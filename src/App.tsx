...
const App: React.FC = () => {
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-3.5 bg-slate-50 border rounded-xl text-center text-lg font-bold tracking-widest focus:outline-none focus:ring-2 transition-all ${loginError
  ...
  return (
    <div className="min-h-screen flex font-sans text-slate-900 overflow-hidden relative">
      <div className="absolute inset-0 bg-[#F5F5F7] -z-10" /> {/* Explicit Background Layer */}
      ...
      {/* Sidebar Container */}
      <div className={`fixed inset-y-0 left-0 z-50 transition-transform duration-300 md:relative md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <Sidebar
          currentView={view}
          onNavigate={(v) => {
            setView(v);
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative z-0">
        {/* Mobile Header */}
        <div className="md:hidden p-4 flex items-center justify-between shrink-0 glass border-b border-slate-200/50 sticky top-0 z-30">
          <h1 className="font-bold text-lg text-slate-900">ARTIMILANO</h1>
          <button onClick={() => setSidebarOpen(true)} className="p-2 rounded-lg hover:bg-black/5 text-slate-600">
            <Menu size={20} />
          </button>
        </div>
