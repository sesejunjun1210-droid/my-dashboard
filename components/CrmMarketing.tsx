...
const CrmMarketing: React.FC<CrmMarketingProps> = ({ data }) => {
  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in duration-500 relative">
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] bg-slate-800 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-2 animate-in slide-in-from-bottom-5 fade-in duration-300 w-max max-w-[90%]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="text-sm text-slate-500">총 고객 수</p>
              <p className="text-2xl font-bold text-slate-900">{retentionData.total}명</p>
            </div>
          </div>
          <p className="text-xs text-slate-400">전화번호 기준 식별된 고객</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-3 mb-4">
          ...
          <p className="text-xs text-slate-400">2회 이상 서비스 이용 고객</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-3 bg-amber-50 rounded-xl text-amber-600"><Crown size={24} /></div>
            <div>
              <p className="text-sm text-slate-500">재방문율</p>
              <p className="text-2xl font-bold text-slate-900">{retentionData.returnRate}%</p>
            </div>
          </div>
          <p className="text-xs text-slate-400">높을수록 고객 만족도가 높습니다.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-blue-50 p-5 sm:p-6 rounded-2xl border border-indigo-100 flex flex-col h-full">
            <div className="flex justify-between items-start mb-4">
                <div>
                    ...
                    <p className="text-xs sm:text-sm text-indigo-600/80 mt-1">
                        정확히 1년 전 서비스를 이용한 고객입니다.
                    </p>
                </div>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[300px] custom-scrollbar pr-1">
                {retentionCalendar.map((customer, idx) => (
                    <div key={idx} className="bg-white p-3 rounded-xl shadow-sm border border-indigo-100 flex justify-between items-center group hover:bg-indigo-50 transition-colors cursor-pointer" onClick={() => setSelectedCustomer(customer)}>
                        <div className="overflow-hidden mr-2">
                            <p className="font-bold text-slate-800 text-sm truncate">{customer.name}</p>
                            <p className="text-xs text-slate-500 mt-0.5 truncate">방문일: {customer.lastVisit}</p>
                        </div>
                        <button 
                            onClick={(e) => handleCopyScript(e, 'retention', customer.name)}
                            className="p-3 bg-indigo-100 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-colors active:scale-95 shrink-0 shadow-sm"
                            title="안부 문자 복사"
                        >
                            <Copy size={18} />
                        </button>
        ...
        <div className="bg-gradient-to-br from-rose-50 to-orange-50 p-5 sm:p-6 rounded-2xl border border-rose-100 flex flex-col h-full">
            <div className="flex flex-col mb-4 gap-3">
                ...
                {churnRiskData.valueAtRisk > 0 && (
                    <div className="bg-white/60 border border-rose-200 rounded-xl p-3 flex items-center gap-3 animate-pulse">
                        <div className="p-2 bg-rose-100 rounded-full text-rose-600 shrink-0">
                            <Coins size={18} />
                        </div>
                        <div className="overflow-hidden">
                            <p className="text-[10px] font-bold text-rose-500 uppercase tracking-wide truncate">Money on the Table (Value at Risk)</p>
                            <p className="text-sm font-bold text-slate-800 truncate">
                                총 <span className="text-rose-600 text-lg">₩ {churnRiskData.valueAtRisk.toLocaleString()}</span> 가치가 이탈 중입니다.
                            </p>
                        </div>
                    </div>
                )}
            ...
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[240px] custom-scrollbar pr-1">
                {churnRiskData.customers.map((customer, idx) => (
                    <div key={idx} className="bg-white p-3 rounded-xl shadow-sm border border-rose-100 flex justify-between items-center group hover:bg-rose-50 transition-colors cursor-pointer" onClick={() => setSelectedCustomer(customer)}>
                        <div className="overflow-hidden mr-2">
                            <div className="flex items-center gap-2 mb-0.5">
                                <p className="font-bold text-slate-800 text-sm truncate">{customer.name}</p>
                                <span className="text-[10px] bg-rose-100 text-rose-600 px-1.5 rounded font-bold whitespace-nowrap">
                                    {customer.visitCount}회
                                </span>
                            </div>
                            <p className="text-xs text-slate-500 mt-0.5 truncate">주기 {customer.avgCycle}일 → <span className="text-rose-500 font-bold">{Math.round((new Date().getTime() - new Date(customer.lastVisit).getTime())/(1000*3600*24))}일째 미방문</span></p>
                        </div>
                        <button 
                            onClick={(e) => handleCopyScript(e, 'churn', customer.name)}
                            className="p-3 bg-rose-100 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition-colors active:scale-95 shrink-0 shadow-sm"
                            title="안부 문자 복사"
                        >
                            <Copy size={18} />
                        </button>
        ...
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="relative w-full sm:w-auto">
             <input 
                ...
                className="w-full sm:w-64 pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-slate-50"
                value={searchTerm}
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 group-hover:shadow-md transition-all">
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold bg-slate-100 text-slate-600 px-2 py-1 rounded mb-1 w-fit">
                            {record.date}
                          </span>
                          <div className="flex items-center gap-2">
